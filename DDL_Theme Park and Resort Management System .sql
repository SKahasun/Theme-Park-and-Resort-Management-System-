...
/*====================================  SECTION 02  ===================================
               CREATE SEQUENCE: Generates a sequence of numeric values 				      
=======================================================================================*/
...
/*=====================================================================================
	     	                          CREATE SCHEMA
======================================================================================*/
-- Create a schema for human resources and staff management
CREATE SCHEMA HR
GO

-- Create a schema for financial and sales records
CREATE SCHEMA Finance
GO

-- Create a schema for park operations (rides, maintenance)
...
/*======================================================================================
	                   CREATE TRIGGER  (AFTER AND INSTEAD OF TRIGGER)
========================================================================================
...
-- INSTEAD OF TRIGGER: Prevents deletion of a Park if it has active rides

CREATE TRIGGER tr_PreventParkDeletion
ON Park
INSTEAD OF DELETE
AS
BEGIN
    ...
    IF EXISTS (
        SELECT 1 
        FROM Rides 
        WHERE ParkID IN (SELECT ParkID FROM deleted) 
        AND Status = 'Operational'
    )
    BEGIN
        RAISERROR('Cannot delete a park with operational rides. Deactivate rides first.', 16, 1);
    END
    ELSE
    BEGIN
        DELETE FROM Parks 
        ...
        PRINT 'Park deleted successfully.';
    END
END
GO


-- INSTEAD OF UPDATE: Protects specific columns from being changed

CREATE TRIGGER tr_ProtectParkLocation
ON Park
INSTEAD OF UPDATE
AS
BEGIN
    ...
    IF EXISTS (
        SELECT 1 
        FROM inserted i
        JOIN deleted d ON i.ParkID = d.ParkID
        WHERE i.Location <> d.Location
    )
    BEGIN
        RAISERROR('The Location of an established Park cannot be changed.', 16, 1);
        ROLLBACK TRANSACTION;
    END
    ELSE
    BEGIN
        UPDATE Parks
        SET ParkName = i.ParkName,
            TotalRides = i.TotalRides,
            OpeningTime = i.OpeningTime,
            ClosingTime = i.ClosingTime
        FROM Parks p
        JOIN inserted i ON p.ParkID = i.ParkID;
        
        PRINT 'Park details updated successfully.';
    END
END
GO
...
--This view provides a secure list of currently active employees, their roles, and contact information
CREATE VIEW dbo.vw_ActiveEmployeeDirectory
...
--This view summarizes financial transactions by type and category, providing a secure 
--way for management to view income and expenses without seeing granular details.
CREATE VIEW dbo.vw_FinancialSummary
WITH SCHEMABINDING
AS
SELECT 
    TransactionType, 
...
--A secure view to manage visitor data, focusing on active members and their contact 
--details for marketing or emergency purposes.
CREATE VIEW dbo.vw_VisitorMembershipContacts
WITH ENCRYPTION
AS
SELECT 
    VisitorID, 
...
--This view offers a consolidated look at rides across different parks, 
--including their capacity and current maintenance status.
CREATE VIEW dbo.vw_ParkRideStatus
AS
SELECT 
    p.ParkName, 
    br.BoatName AS RideName, 
    'Boat Ride' AS RideType, 
    br.Capacity, 
    br.OperatingHours
FROM dbo.BoatRides br
JOIN dbo.Park p ON br.ParkID = p.ParkID
UNION ALL
SELECT 
    p.ParkName, 
    hr.HorseName, 
    ...
    hr.Capacity, 
    hr.OperatingHours
FROM dbo.HorseRides hr
JOIN dbo.Park p ON hr.ParkID = p.ParkID
GO


--This view displays all resort rooms that are currently marked as 'Available', 
--helping front-desk staff identify vacancies quickly.
CREATE VIEW dbo.vw_AvailableResortRooms
WITH SCHEMABINDING, ENCRYPTION
AS
SELECT 
    rr.RoomID, 
    ...
    rr.Capacity, 
    rr.RatePerNight
FROM dbo.ResortRoom rr
JOIN dbo.Resort r ON rr.ResortID = r.ResortID
WHERE rr.AvailabilityStatus = 'Available';
GO
...
--This example targets the Visitors table, including common contact and membership details.
...
AS
BEGIN
    INSERT INTO dbo.Visitors (VisitorID, FullName, Age, PhoneNumber, VisitorType, VisitPurpose, MembershipStatus)
    VALUES (@VisitorID, @FullName, @Age, @PhoneNumber, @VisitorType, @VisitPurpose, @MembershipStatus);
END
GO


--Since the FinancialTransactions table uses an IDENTITY column for the TransactionID, 
--you do not include it in the VALUES list as the system generates it automatically.
CREATE PROCEDURE sp_InsertFinancialTransaction
...
AS
BEGIN
    INSERT INTO dbo.FinancialTransactions (TransactionType, Category, Amount, PaymentMethod, Description)
    VALUES (@TransactionType, @Category, @Amount, @PaymentMethod, @Description)
END
GO

--This procedure allows you to add a new park to the Park table.
CREATE PROCEDURE sp_InsertPark
    @ParkID INT,
    @ParkName VARCHAR(100),
    @Location VARCHAR(150),
    @Address VARCHAR(250),
...
AS
BEGIN
    INSERT INTO dbo.Park (ParkID, ParkName, Location, Address, ParkType, EntryFee)
    VALUES (@ParkID, @ParkName, @Location, @Address, @ParkType, @EntryFee)
END
GO
...
--This procedure calculates the total revenue for a specific park from TicketSales 
--and returns the result through an OUTPUT parameter.
CREATE PROCEDURE sp_GetParkTotalRevenue
    @ParkID INT,
...
BEGIN
    ...
    IF @TotalRevenue IS NULL SET @TotalRevenue = 0.00
END
GO


--It handles a single task (adding a visitor), includes a TRY...CATCH block for safety
CREATE PROCEDURE sp_AddVisitorSecure
    @ID INT, 
    @Name VARCHAR(100), 
    @Type VARCHAR(50)
AS
BEGIN
    BEGIN TRY
        INSERT INTO dbo.Visitors (VisitorID, FullName, VisitorType)
        VALUES (@ID, @Name, @Type)

        SELECT * FROM dbo.Visitors WHERE VisitorID = @ID
...
--Multi-Action Stored Procedure
CREATE PROCEDURE dbo.sp_VisitorMasterControl
    @Action VARCHAR(10),      
    @VisitorID INT,
...
BEGIN
    ...
    BEGIN TRY
        IF @Action = 'INSERT'
        BEGIN
            INSERT INTO dbo.Visitors (VisitorID, FullName, Age, VisitorType)
            VALUES (@VisitorID, @FullName, @Age, @VisitorType)
        ...
        BEGIN
            ...
            SET FullName = ISNULL(@FullName, FullName),
                Age = ISNULL(@Age, Age),
                VisitorType = ISNULL(@VisitorType, VisitorType)
        ...
        BEGIN
            DELETE FROM dbo.Visitors WHERE VisitorID = @VisitorID
            PRINT 'Visitor record deleted.'
        END
        IF @Action <> 'DELETE'
        BEGIN
            SELECT * FROM dbo.Visitors WHERE VisitorID = @VisitorID
        END
        ELSE
        BEGIN
            SELECT 'Record Deleted' AS Status, @VisitorID AS ID
        END
    ...
    BEGIN CATCH
        SELECT 
            ERROR_MESSAGE() AS ErrorMessage,
            'Action Failed: ' + @Action AS FailedAction
    END CATCH
END
GO
...
--Multi-Statement Table-Valued Function: fn_GetDetailedParkReport
CREATE FUNCTION dbo.fn_GetDetailedParkReport
(
    @ParkID INT
)
...
AS
BEGIN
    DECLARE @PName VARCHAR(100), @EmpCount INT, @RideCount INT = 0
    DECLARE @TicketCount INT, @Revenue DECIMAL(18, 2)

    SELECT @PName = ParkName FROM dbo.Park WHERE ParkID = @ParkID
    SELECT @EmpCount = COUNT(EmployeeID) FROM dbo.Employees WHERE ParkID = @ParkID

    SET @RideCount += (SELECT COUNT(*) FROM dbo.BoatRides WHERE ParkID = @ParkID)
    SET @RideCount += (SELECT COUNT(*) FROM dbo.HorseRides WHERE ParkID = @ParkID)
    SET @RideCount += (SELECT COUNT(*) FROM dbo.KidTrainRide WHERE ParkID = @ParkID)
    SET @RideCount += (SELECT COUNT(*) FROM dbo.FerrisWheel WHERE ParkID = @ParkID)

    SELECT 
        @TicketCount = ISNULL(SUM(Quantity), 0),
        @Revenue = ISNULL(SUM(TotalAmount), 0)
    FROM dbo.TicketSales 
    WHERE ParkID = @ParkID

    INSERT INTO @ReportTable (ParkID, ParkName, TotalEmployees, TotalRideCount, TotalTicketsSold, TotalRevenue)
    VALUES (@ParkID, @PName, @EmpCount, @RideCount, @TicketCount, @Revenue)
    RETURN
END
GO
...
---- Adding a new column to the Park table
--ALTER TABLE dbo.Park 
--ADD IsActive BIT DEFAULT 1
--GO

---- Dropping a column from the Employees table
...
ALTER SEQUENCE seq_InvoiceNumber
    RESTART WITH 2000
    INCREMENT BY 5
GO
...
-- Rebuilding an index for better performance
ALTER INDEX NCI_Resort_ParkID ON Resort REBUILD
GO

-- Disabling an index
ALTER INDEX NCI_Employees_GradePark ON Employees DISABLE
GO
...
ALTER VIEW dbo.vw_ActiveEmployeeDirectory 
WITH SCHEMABINDING, ENCRYPTION 
AS 
SELECT EmployeeID, FullName, Role, Department, PhoneNumber, Email, HireDate, ShiftSchedule
FROM dbo.Employees 
WHERE EmploymentStatus = 'Active'
GO
...
ALTER PROCEDURE sp_UpdateRoomStatus 
@RoomID INT, 
@NewStatus VARCHAR(50) 
AS 
BEGIN 
...
ALTER FUNCTION dbo.fn_CalculateEmployeeNetSalary (@EmployeeID INT)
RETURNS DECIMAL(10, 2)
AS
BEGIN
    DECLARE @NetSalary DECIMAL(10, 2) = 0
    SELECT @NetSalary = Salary + Bonus FROM Employees WHERE EmployeeID = @EmployeeID
    RETURN @NetSalary
END
GO
...
ALTER TRIGGER trg_UpdateStock_AfterSale 
ON ShopSales 
AFTER INSERT 
AS 
BEGIN 
    SET NOCOUNT ON;
    UPDATE p
    SET p.StockQuantity = p.StockQuantity - i.QuantitySold
    FROM dbo.Products AS p
    INNER JOIN inserted AS i ON p.ProductID = i.ProductID;
    IF EXISTS (SELECT 1 FROM dbo.Products WHERE StockQuantity < 0)
    BEGIN
        PRINT 'Warning: One or more products have reached negative stock levels.';
    END

    PRINT 'Stock levels updated successfully based on recent sales.'
END
GO
...
--DROP TRIGGER IF EXISTS trg_UpdateStock_AfterSale
--DROP TRIGGER IF EXISTS trg_UpdateRoomStatus
--DROP TRIGGER IF EXISTS trg_LogExpense_Transaction
--DROP TRIGGER IF EXISTS trg_LogShopSales_Income
--DROP TRIGGER IF EXISTS trg_LogResortBooking_Income
--DROP TRIGGER IF EXISTS tr_PreventParkDeletion
--GO
...
--DROP FUNCTION IF EXISTS dbo.fn_CalculateEmployeeNetSalary
--DROP FUNCTION IF EXISTS dbo.fn_GetParkAttendanceSummary
--DROP FUNCTION IF EXISTS dbo.fn_GetDetailedParkReport
--GO
...
--DROP PROCEDURE IF EXISTS dbo.sp_AddShopSale
--DROP PROCEDURE IF EXISTS dbo.sp_UpdateRoomStatus
--GO
...
--DROP VIEW IF EXISTS dbo.vw_ActiveEmployeeDirectory
--DROP VIEW IF EXISTS dbo.vw_AssetMaintenanceSummary
--DROP VIEW IF EXISTS dbo.vw_AvailableResortRooms
--GO
...
--DROP INDEX IF EXISTS NCI_Resort_ParkID ON Resort
--DROP INDEX IF EXISTS NCI_Employees_GradePark ON Employees
--DROP INDEX IF EXISTS NCI_FinancialTransactions_ReportFilter ON FinancialTransactions
--GO


/*==================================================================================
                 DROP TABLES (Ordered to handle Foreign Key constraints)
====================================================================================*/

--DROP TABLE IF EXISTS IncidentReport
--DROP TABLE IF EXISTS CustomerFeedback
--DROP TABLE IF EXISTS ResortBookings
--DROP TABLE IF EXISTS AttendanceStaffShift
--DROP TABLE IF EXISTS ShopSales
--DROP TABLE IF EXISTS FishSales
--DROP TABLE IF EXISTS FruitSales
--DROP TABLE IF EXISTS FlowerPlant
--DROP TABLE IF EXISTS FishFarms
--DROP TABLE IF EXISTS FruitGarden
--DROP TABLE IF EXISTS Shop
--DROP TABLE IF EXISTS PlaygroundFacilities
...
--DROP SEQUENCE IF EXISTS seq_InvoiceNumber
--GO
